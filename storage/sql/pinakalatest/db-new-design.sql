...
CREATE TABLE `members` (
  ...
  `approval_status` enum('approved','pending','rejected') COLLATE utf8mb4_general_ci DEFAULT 'pending',
  `current_balance` decimal(10,2) DEFAULT '0.00',
  `first_name` varchar(50) COLLATE utf8mb4_general_ci NOT NULL,
  `middle_name` varchar(50) COLLATE utf8mb4_general_ci DEFAULT NULL,
  `last_name` varchar(50) COLLATE utf8mb4_general_ci NOT NULL,
  `sex` enum('M','F') COLLATE utf8mb4_general_ci NOT NULL DEFAULT 'M',
  `contact_number` varchar(20) COLLATE utf8mb4_general_ci NOT NULL,
...
CREATE TABLE `member_types` (
  ...
  `type_name` varchar(50) COLLATE utf8mb4_general_ci NOT NULL, -- Loan, Savings Accounts, Fixed Deposit, etc.
...
CREATE TABLE `cooperative_accounts` (
  ...
  `created_at` timestamp NULL DEFAULT CURRENT_TIMESTAMP,
  `updated_at` timestamp NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  `opened_date` date NOT NULL DEFAULT '2025-05-11',
  `closed_date` date DEFAULT NULL,
  `status` enum('pending','active','inactive','suspended','closed') COLLATE utf8mb4_general_ci DEFAULT 'active',
  PRIMARY KEY (`caid`),
  KEY `fk_caid_type_id` (`type_id`),
  KEY `fk_caid_member_id` (`member_id`),
  CONSTRAINT `fk_caid_member_id` FOREIGN KEY (`member_id`) REFERENCES `members` (`member_id`) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT `fk_caid_type_id` FOREIGN KEY (`type_id`) REFERENCES `member_types` (`type_id`)
) ENGINE=InnoDB AUTO_INCREMENT=50 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;
...
CREATE TABLE `member_amortizations` (
  ...
  `status` enum('paid','pending','overdue','defaulted') CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci DEFAULT NULL,
  `created_at` timestamp NULL DEFAULT CURRENT_TIMESTAMP,
  `updated_at` timestamp NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
...
CREATE TABLE `member_transactions` (
  ...
  `transaction_type` enum('deposit','withdrawal','interest','fee','credit','credit_used','loan_payment') COLLATE utf8mb4_general_ci DEFAULT NULL,
  `amount` decimal(10,2) NOT NULL,
  ...
  KEY `fk_mt_caid` (`cooperative_id`),
  CONSTRAINT `fk_mt_caid` FOREIGN KEY (`cooperative_id`) REFERENCES `cooperative_accounts` (`caid`),
