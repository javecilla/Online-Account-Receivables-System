...
const updateMonthlyBalanceTrendsChart = (year) => {
    ...
    axios
      ...
      .then((response) => {
        if (response.data.success) {
          ...
          reject(new Error('Failed to get monthly balance trends data'))
        }
      })
      .catch((error) => {
...
const getAccountTypeColor = (type) => {
  const colors = {
    ...
    'Time Deposit': getComputedStyle(document.documentElement).getPropertyValue(
      '--info-color'
    )
  }
...
yearSelect.addEventListener('change', (e) => {
  updateMonthlyBalanceTrendsChart(e.target.value)
})

// Initial chart load
//updateMonthlyBalanceTrendsChart(2025)
window.updateMonthlyBalanceTrendsChart = updateMonthlyBalanceTrendsChart
